...
- 메인 화면 > `정책 생성` 버튼 선택
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-00-02.png){: width="30%"}


- 정색 생성 화면 > JSON 탭 선택 > 정책 내용 입력(아래 코드 참고) > `정책 검토` 버튼 선택 

    ![](/blog/assets/images/cloud/aws/aws-cicd03-00-03.png)
...
- 정책 이름을 `CodeBuildECRPolicy-cicd-demo-ap-northeast-2`로 입력 > `정책 생성` 버튼 선택 
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-00-04.png)
...
- install : Java SDK, Docker CLI를 설치
- pre_build : ECR 서비스에 로그인
- build : Maven 및 Docker 빌드
- post_build : Docker 이미지를 ECR에 push
...
- 빌드 배지 활성화 체크
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-02.png)
...
- 리포지토리 `cicd-demo` 선택
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-03.png)
...
- 권한이 있음 체크 
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-04.png)
...
- 역할 이름 `codebuild-cicd-demo-service-role` 입력
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-05.png)

##### 추가 구성

- 추가 구성 메뉴 확장 > 환경 변수 입력

    ![](/blog/assets/images/cloud/aws/aws-cicd03-06-00.png)

    ![](/blog/assets/images/cloud/aws/aws-cicd03-06-01.png){: width="70%"}
...
- 빌드 사양 > `buildspec 파일 사용` 선택
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-07.png)

#### 아티팩트

- 유형 > `아티팩트 없음` 선택

    ![](/blog/assets/images/cloud/aws/aws-cicd03-08.png)
...
- `빌드 프로젝트 생성` 버튼 선택
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-09.png)
...
- 메인 화면 > 검색 창 > `codebuild` 입력 > `codebuild-cicd-demo-service-role` 선택
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-10-00.png){: width="50%"}

- 권한 탭 > `정책 연결` 버튼 선택

    ![](/blog/assets/images/cloud/aws/aws-cicd03-10-01.png){: width="50%"}

- 정책 필터 > `codebuild` 입력 > `CodeBuildECRPolicy-cicd-demo-ap-northeast-2` 선택 > `정책 연결` 버튼 선택

    ![](/blog/assets/images/cloud/aws/aws-cicd03-10-02.png)
...
- 메인 화면 > `cicd-demo` 프로젝트 선택 > `빌드 시작` 버튼 선택 
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-11-00.png)

- 빌드가 성공적으로 종료되면 상태가 성공함으로 변경됩니다

    ![](/blog/assets/images/cloud/aws/aws-cicd03-11-01.png)

- ECR의 `cicd-demo` 저장소에 접속해보면 새로운 이미지가 생성되어 있습니다.

    ![](/blog/assets/images/cloud/aws/aws-cicd03-11-02.png){: width="60%"}
...
![](/blog/assets/images/cloud/aws/aws-cicd03-11-03.png)

빌드 로그 탭을 선택해 로그를 확인해보면 Maven Build를 하기 위해 의존성 라이브러리를 다운로드하는데 많은 시간이 소요되는 것을 확인할 수 있습니다.

![](/blog/assets/images/cloud/aws/aws-cicd03-11-04.png)
...
- 아티팩트 > `편집` 버튼 선택
    ...
    ![](/blog/assets/images/cloud/aws/aws-cicd03-12-00.png)

- 추가 구성 메뉴 확장 > 캐시 유형 `Amazon S3` 선택 > 캐시 버킷 선택(S3 콘솔에서 생성)

    ![](/blog/assets/images/cloud/aws/aws-cicd03-12-02.png)

설정을 완료한 후 다시 빌드를 시작합니다. 빌드가 완료된 후 기록을 보겠습니다. 
아래와 같이 기존 `14분 55초`에서 `1분 54초로` 빌드 시간이 단축되었습니다.

![](/blog/assets/images/cloud/aws/aws-cicd03-12-03.png)

단계 세부 정보를 확인해보면 BUILD 단계가 기존 `14분`에서 `1분`으로 단축되었습니다.

![](/blog/assets/images/cloud/aws/aws-cicd03-12-04.png)